...
import { getTime12hrs, patchData } from "../../../helperfunction";
import RatingCard from "./RatingCard";
import { generatePDF } from "./generatebill";

function OrderCard({ orderDetail, user, onOrderCancelled }) {
  ...
  const handleCancelOrder = async () => {
    if (cancelLoading || orderDetail.status === "Cancelled") return;
    setCancelLoading(true);
    const res = await patchData("order/cancel-order", { orderId: orderDetail._id });
    setCancelLoading(false);
    if (res?.success) {
      onOrderCancelled?.(orderDetail._id);
    }
  };
      ...
      {view === "rating" ? (
        ...
        <div className="menu-container detailed-order">
          {orderDetail.orders.map((order, index) => (
            <div className="dish-box" key={order._id}>
              ...
              <div className="order-timeline">
                <div
                  className={`timeline-status ${setStatus(order.status, 1)}`}
                >
                  <span>
                  ...
                  <span className="status-time-created">
                    {getTime12hrs(orderDetail.createdAt)}
                  </span>
                </div>
                <div
                  className={`timeline-status ${setStatus(order.status, 2)}`}
                >
                  <span>
                  ...
                  <span className="status-time">
                    {getTime12hrs(orderDetail.updatedAt)}
                  </span>
                </div>
                <div
                  className={`timeline-status ${setStatus(order.status, 3)}`}
                >
                  <span>
                    <img src={prepared}></img>
                  </span>
                  <div>
                    <div className="status-dot"></div>
                    <div className="status-line"></div>
                ...
                <div
                  className={`timeline-status ${setStatus(order.status, 4)}`}
                >
                  <span>
                    <img src={served}></img>
                  </span>
          ...
          <div className="accounting">
            ...
            <button
              className="ordercard-btn"
              onClick={handleCancelOrder}
              disabled={cancelLoading || orderDetail.status === "Cancelled"}
            >
              {cancelLoading ? (
                <>
            ...
            <button onClick={toggleView}>Close Order</button>
          </div>
        </div>
      ) : (
        <div className="order-card">
            ...
            <div className="order-status">
              ...
              {orderDetail.status == "Served" && (
                <i
                  ...
                  onClick={() => generatePDF(orderDetail, user)}
          ...
          <div className="card-bottom">
            <div className="order-basics">
              <p>
                {orderDetail.createdAt
                  .toString()
                  .slice(0, 10)
                  .concat(" ", getTime12hrs(orderDetail.createdAt))}
              </p>
            ...
            <div className="order-basics order-btns">
              <button className="ordercard-btn" onClick={toggleView}>
                View Order
              </button>
              <button
                className="ordercard-btn"
